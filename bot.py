...
def get_user(tg_id):
    ...
    # ADMIN_TG_ID робить адміністратора стійким до перезапусків/нового контейнера Railway.
...
async def admin_reports(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not u or u["role"]!="admin":
        await update.message.reply_text("⚠️ Адміністратор не авторизований після перезапуску. Введи /admin СЕКРЕТ один раз.")
        return
